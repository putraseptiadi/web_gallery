...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/admin/manage_tkro.css') }}">
    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 p-8">
            ...
            <div class="flex justify-between items-center border-b pb-4 mb-8">
                <h1 class="text-3xl font-bold">Manage TKRO</h1>
            ...
            <a href="{{ route('jurusan_admin.manage_propres_tkro') }}" class="text-blue-600 hover:text-blue-800 flex items-center mb-4">
                Lihat Manage propres tkro
            ...
            <div class="container">
                <form action="{{ route('tkro_program.store') }}" method="POST" enctype="multipart/form-data" class="form-ketua-program">
                    @csrf
            ...
            <section class="ketua-program-section">
                @foreach($KetuaTkro as $KetuaTkro)
                <div class="ketua-program-card mb-3">
                    <div class="ketua-program-row">
                        <div class="ketua-program-image">
                            <img src="{{ asset('storage/' . ($KetuaTkro->foto ?? 'path/images/pp_kosong.jpg')) }}"
                        ...
                        <div class="ketua-program-content">
                            <div class="ketua-program-body">
                                <h5 class="ketua-program-title"><strong>Nama:</strong> {{ $KetuaTkro->nama }}</h5>
                                <p class="ketua-program-title"><strong>Bidang Keahlian:</strong> {{ $KetuaTkro->bidang_keahlian }}</p>
                                <div class="ketua-program-buttons">
                                    <a href="{{ route('tkro_program.edit', $KetuaTkro->id) }}" class="btn btn-edit">Edit</a>
                                    <form action="{{ route('tkro_program.destroy', $KetuaTkro->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
