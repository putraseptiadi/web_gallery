...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/admin/propres_tflm.css') }}">
    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 p-8">
            ...
            <div class="flex justify-between items-center border-b pb-4 mb-8">
                <h1 class="text-3xl font-bold">Profesi & Prestasi TFLM</h1>
            ...
            <section class="program-info container my-5">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <form action="{{ route('prestasitflm.store') }}" method="POST" class="form-bg">
                            @csrf
                        ...
                        <table class="table table-striped mt-4">
                            ...
                            <tbody>
                                @foreach($PrestasiTflm as $index => $prestasi)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $prestasi->judul }}</td>
                                    <td>{{ $prestasi->tahun }}</td>
                                    <td>
                                        <a href="{{ route('prestasitflm.edit', $prestasi->id) }}" class="btn btn-warning btn-sm">Edit</a>
                                        <form action="{{ route('prestasitflm.destroy', $prestasi->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <div class="col-md-6">
                        ...
                        <form action="{{ route('profesitflm.store') }}" method="POST" class="form-bg">
                            @csrf
                            <div class="form-group">
                        ...
                        <table class="table table-striped mt-4">
                            ...
                            <tbody>
                                @foreach($ProfesiTflm as $index => $profesi)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $profesi->judul }}</td>
                                    <td>
                                        <a href="{{ route('profesitflm.edit', $profesi->id) }}" class="btn btn-warning btn-sm">Edit</a>
                                        <form action="{{ route('profesitflm.destroy', $profesi->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus data ini?')">Hapus</button>
                                        </form>
