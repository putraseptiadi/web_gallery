...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/admin2/edit_eskul.css') }}">
...
<body>
    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 p-8">
            ...
            <!-- Tombol Kembali -->
            <a href="{{ route('daftar_program') }}" class="text-blue-600 hover:text-blue-800 flex items-center mb-4">
            ...
            <form action="{{ route('update_job', $job->id) }}" method="POST" class="form-update-job">
                @csrf
                @method('PUT')

                <label for="title">Judul Pekerjaan</label>
                <input type="text" name="title" value="{{ $job->title }}" required class="input-field">

                <label for="company">Nama Perusahaan</label>
                <input type="text" name="company" value="{{ $job->company }}" required class="input-field">

                <label for="description">Deskripsi</label>
                <textarea name="description" required class="input-field">{{ $job->description }}</textarea>

                <label for="location">Lokasi</label>
                <input type="text" name="location" value="{{ $job->location }}" required class="input-field">

                <label for="deadline">Batas Waktu</label>
                <input type="date" name="deadline" value="{{ $job->deadline }}" required class="input-field">
