...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/admin2/daftar_program.css') }}">
...
<body>
    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 p-8">
            ...
            <!-- Tombol Kembali -->
            <a href="{{ route('manage_program') }}" class="text-blue-600 hover:text-blue-800 flex items-center mb-4">
                ...
                <table class="table-jobs">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $job->company }}</td>
                            <td>{{ $job->location }}</td>
                            <td class="job-description">{{ \Illuminate\Support\Str::limit($job->description, 50) }}</td>
                            <td>{{ \Carbon\Carbon::parse($job->deadline)->format('d F Y') }}</td>
                            <td class="action-buttons">
                                <a href="{{ route('edit_job', $job->id) }}" class="btn-edit">Edit</a>
                                <form action="{{ route('delete_job', $job->id) }}" method="POST" class="delete-form">
                                    @csrf
                                    @method('DELETE')
